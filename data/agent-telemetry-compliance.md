...
| 11 | MAF .NET | `OpenAIClient.GetResponsesClient` | Foundry RAPI | 🟢 | OK | OK | OK | OK | **chat:** `gen_ai.response.model` 🟡 reports deployment alias (e.g. `deployment-gpt-4o-mini`) instead of the served model snapshot (e.g. `gpt-4o-mini-2024-07-18`) — Foundry returns the snapshot only in the `x-ms-served-model` response header. MAF Python reads that header in `agent_framework_openai/_chat_client.py` (`SERVED_MODEL_HEADER`, `_extract_served_model`), but the equivalent has not been ported to .NET yet — grep of `Microsoft.OpenTelemetry` 1.0.3, `Microsoft.Extensions.AI.OpenAI` 10.6.0, `Microsoft.Agents.AI[.OpenAI]` 1.7.0, `OpenAI` 2.10.0, `Azure.AI.OpenAI` 2.9.0-beta.1 has zero hits for `served-model` / `ServedModel`. Cannot be fixed via a version bump; upstream code does not exist. `gen_ai.response.finish_reasons` 🟡 Missing on streaming spans only; `gen_ai.request.choice.count` 🟡 Missing; `gen_ai.conversation.id` 🟠 Missing<br>**invoke_agent:** all Required ✓; `gen_ai.conversation.id` 🟠 Missing; `gen_ai.agent.version` 🟡 Missing |
