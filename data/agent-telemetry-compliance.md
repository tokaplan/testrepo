...
| MAF .NET | `Microsoft.OpenTelemetry 1.0.3` + `Microsoft.Agents.AI 1.7.0` | ✅ works — all chat / execute_tool / HTTP / nested `invoke_agent` spans inherit | ✅ works — root `MainWeatherAgent` and `VerifierAgent` `invoke_agent` spans self-promote `gen_ai.agent.name → microsoft.gen_ai.main_agent.name` | ⚠ partial — `name` + `id` present on **100%** of spans (44/44 across both protocols on `wk2-mafnet-125304` — 11/11 chat, 8/8 execute_tool, 6/6 invoke_agent, 19/19 HTTP/GetToken); `version` 0/all (no `gen_ai.agent.version` to fall back from); `conversation_id` 6/19 on HTTP-class spans | ✅ **Fully compliant for `name` + `id`** — newly implemented in `1.0.3`. Was "Not implemented" in `1.0.2`. |
...
| MAF .NET | `wk2-mafnet-125304` | `Microsoft.OpenTelemetry 1.0.3`, `Microsoft.Agents.AI 1.7.0`, `Microsoft.Agents.AI.Workflows 1.7.0` |
