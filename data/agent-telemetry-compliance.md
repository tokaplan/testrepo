...
|---|---|:-:|:-:|:-:|---|
| MAF Python | `microsoft-opentelemetry 1.3.2` | ✅ works — `chat` 12/13 (92%) and `execute_tool` 8/9 (89%) inherit `main_agent.{name,id}` from parent's `gen_ai.agent.*` (no material change from `1.3.0` — minor noise from added HTTP spans) | ❌ root Main + Verifier `invoke_agent` spans still never get `main_agent.*` (only 2/7 invoke_agent spans attributed on `v132-mafpy-123027` — the nested `WeatherDataAgent` siblings, via OnStart from a chat/tool parent) | ⚠ partial — `name` + `id` present on most child spans, `version` 0/all, `conversation_id` partial | **Partial — unchanged in `1.3.2`** — `1.3.0` closed the child-inheritance gap; the root `invoke_agent` OnEnd self-promotion is still not implemented in `1.3.2`. |
...
| LangChain Python | `microsoft-opentelemetry 1.3.2` | ✅ works — chat / execute_tool / nested `invoke_agent` / HTTP all inherit `main_agent.{name,id}` from the parent's `gen_ai.agent.*` (75/90 spans across the 3-protocol `v132-lcpy-123027` run = **83.3%** coverage; the 15-span gap is the LangGraph workflow infrastructure spans — outer wrapper + intermediate `main`/`verify`/`tools`/`model` chain spans — which have no source `gen_ai.agent.*` to inherit from) | ✅ works — 3 named root `invoke_agent` spans per protocol (Main, Data, Verifier) self-promote `gen_ai.agent.{name,id} → microsoft.gen_ai.main_agent.*` | ⚠ partial — `name` + `id` + `description` present on **inner** invoke_agent spans (3/6 per protocol) and propagated to most descendants; outer `LangGraph` wrappers still emit no `gen_ai.agent.*`; `conversation_id` 0/all | ✅ **Newly working in `1.3.2`** — was "Broken upstream" in `1.3.0` (0% coverage). The fix landed in the bundled `opentelemetry-instrumentation-langchain` that ships with `microsoft-opentelemetry 1.3.2`. |
...
- ✅ With **MAF Python (`microsoft-opentelemetry 1.3.2`)**, customers can filter or aggregate **~90% of `chat` / `execute_tool` / HTTP / nested `invoke_agent`** spans by `microsoft.gen_ai.main_agent.name` (essentially unchanged from `1.3.0`'s 100% — the small dip on `v132-mafpy-123027` is from a handful of HTTP spans the OnStart processor didn't tag in this run; the child-inheritance gap closed in `1.3.0` is still closed). 
- ❌ With **MAF Python**, the two **root `invoke_agent`** spans per workflow run (Main + Verifier) are still NOT included in such a filter — the OnEnd self-promotion gap is still present in `microsoft-opentelemetry 1.3.2`. Workarounds: filter on `gen_ai.agent.name` for those rows, or wait for an OnEnd fix in the Python distro.
- ✅ With **LangChain Python (`microsoft-opentelemetry 1.3.2`)**, customers can now filter or aggregate **~83% of spans** in a multi-agent trace by `microsoft.gen_ai.main_agent.name`. This is a brand-new capability — `1.3.0` had **0% coverage** because the LangChain instrumentor emitted no `gen_ai.agent.name` for the SpanProcessor to read. `1.3.2`'s bundled instrumentor now surfaces the names set in `create_agent(name=...)` and reads `agent_id` / `agent_description` from per-invoke `RunnableConfig.metadata`, so both OnStart inheritance and OnEnd self-promotion now work for the 3 named agents per protocol. The remaining ~17% gap is the LangGraph workflow infrastructure spans (outer wrapper + intermediate `main`/`verify`/`tools`/`model` chain spans) which have no `gen_ai.agent.*` source attribute.
...
### Reference runs (latest, after Python distro upgrade to `1.3.2`)

| Distro | runId | Distro package(s) |
|---|---|---|
| MAF Python | `v132-mafpy-123027` | `microsoft-opentelemetry 1.3.2`, `agent-framework 1.6.0` |
| MAF .NET | `v4-mafnet-130929` | `Microsoft.OpenTelemetry 1.0.3`, `Microsoft.Agents.AI 1.7.0`, `Microsoft.Agents.AI.Workflows 1.7.0` |
| LangChain Python | `v132-lcpy-123027` | `microsoft-opentelemetry 1.3.2`, `langchain 1.2.16`, `langgraph 1.1.10` |
| LangChain NodeJs | `verifagent-lcnode-154632` | `@microsoft/opentelemetry 1.1.0`, `@langchain/langgraph 0.4.0` |

Earlier baselines for comparison: `v3-mafpy-150803` (MAF Py on `1.3.0`), `v5-lcpy-150231` (LC Py on `1.3.0`), `v2-mafpy-112454` (MAF Py on `1.2.0`), `v2-lcpy-112454` (LC Py on `1.2.0`).
